...
## Output er en .sql fil der dropper skemaet.
# Changelog:
# ---------------------------------------------------------------
...
def drop_schema(folder, domain, notebook_filename):
    ...
    for env in ['d', 't', 'p']:
        drop = notebook_str[0:match.span()[0]]+ f"DROP SCHEMA IF EXISTS dap_{env}_gold_{match.group(1)}.{match.group(2)} CASCADE;"
